		...
		<table class="table table-bordered table-hover table-checkable" id="kt_table_1">
					<thead>
						<tr>
							...
							$sql = runloopQuery("SELECT * FROM campaigns_users where campaign_id = '".$compaindetails['ID']."' and campaignexcell_id = '".$compainsheetdetails['ID']."' order by ID desc");
								   }else{
							$sql = runloopQuery("SELECT * FROM campaigns_users where campaign_id = '".$compaindetails['ID']."' order by ID desc");
								   }
